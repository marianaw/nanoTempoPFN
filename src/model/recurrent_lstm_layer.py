...
class mLSTMWeavingLayer(nn.Module):
    ...
    def __call__(self, x: chex.Array, c_state: chex.Array, n_state: chex.Array, m_state: chex.Array, train: bool = False) -> chex.Array:
        ...
        learnable_skip = self.param(
            "learnable_skip", nn.initializers.ones, (x_mlstm_conv_act.shape[-1],))
        learnable_skip = jnp.broadcast_to(
            learnable_skip, x_mlstm_conv_act.shape)
        h_tilde_state_skip = h_tilde_state + \
            (learnable_skip * x_mlstm_conv_act)
        ...
        y = nn.Dense(
            ...
            dtype=self.config._dtype,
            kernel_init=wang_init(
                x.shape[-1], num_blocks=self.config._num_blocks),
            use_bias=self.config.bias,
            name="proj_down",
        )(h_state)
